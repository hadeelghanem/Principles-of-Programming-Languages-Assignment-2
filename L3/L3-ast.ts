...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x)  || isClassExp(x);  //added  isClassExp(x)
...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    // check that fields and bindings are valid
    ...
    // parse methods
    const methodNames = map(x => x[0], methods);
    const parsedMethodsExp = mapResult(parseL3CExp, map(second, methods));
    const parsedMethods = mapv(parsedMethodsExp, (lambda: CExp[]) => zipWith(makeBinding, methodNames, lambda));
    // bind fields and methods
    return bind(parsedMethods, (bindings: Binding[]) => makeOk(makeClassExp(parsedFields, bindings)));
}
...
const unparseClassExp = (ce: ClassExp) : string =>
    `(class (${ce.fields.map(field => field.var).join(" ")}) (${ce.methods.map(method => `(${method.var.var} ${unparseL3(method.val)})`).join(" ")}))`;
