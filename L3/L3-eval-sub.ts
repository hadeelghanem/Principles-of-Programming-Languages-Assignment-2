...
import { isCExp, isLetExp ,Binding, ClassExp,isClassExp, isCompoundExp } from "./L3-ast";
...
import { isClosure, makeClosure, Closure, Value ,Class,makeClass,Object,makeObject,
            isClass,isObject, SExpValue, isSymbolSExp} from "./L3-value";
...
const evalClass = (ce: ClassExp, env: Env): Result<Class> =>
    makeOk(makeClass(ce.fields, ce.methods));
...
//--------------Added ----------------
const applyClass = (classValue: Class, args: Value[], env: Env): Result<Value> => {
...
const applyObject = (object: Object, args: Value[], env: Env): Result<Value> => {
    ...
    if (!isSymbolSExp(args[0])) {
        return makeFailure("First argument isn not a method name")
    }
