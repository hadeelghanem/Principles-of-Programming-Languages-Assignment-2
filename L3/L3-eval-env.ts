...
import { isClosure, makeClosureEnv, Closure,Class , Object ,isClass,isObject,makeClassEnv,makeObjectEnv, Value, isSymbolSExp } from "./L3-value";
...
const evalClass = (exp: ClassExp, env: Env): Result<Class> =>       //Added
...
const applyClass = (classVal: Class, args: Value[]): Result<Value> => 
...
const applyObject = (obj: Object, args: Value[]): Result<Value> => {
    ...
    const methodNames = map((b: Binding) => b.var.var , obj.methods);

    if(!methodNames.includes(selectedName)) {
        return makeFailure(`Unrecognized method: ${selectedName}`)
    }
    const selectedMethod = obj.methods[methodNames.indexOf(selectedName)];
